...
public class MagicObelisk implements Actionable{
    ...
    public static MagicObelisk createMagicObelisk(String id, Point position, double animationPeriod, List<PImage> images, int time) {
        return new MagicObelisk(id, position, images,  animationPeriod, time);
    }

    public MagicObelisk(String id, Point position, List<PImage> images, double animationPeriod, int time)
    {
        this.id = id;
    ...
    private void swap_reg_background(Point current, WorldModel world, ImageStore imageStore){
        if (world.getBackgroundCell(current).getImages().equals(imageStore.getImageList("magicGrass"))) {
            world.setBackgroundCell(current, new Background("grass", imageStore.getImageList("grass")));
        }
        else if (world.getBackgroundCell(current).getImages().equals(imageStore.getImageList("magicFlowers"))) {
            world.setBackgroundCell(current, new Background("flowers", imageStore.getImageList("flowers")));
        }
        else if (world.getBackgroundCell(current).getImages().equals(imageStore.getImageList("magic_dirt_bot_left_corner"))) {
            world.setBackgroundCell(current, new Background("dirt_bot_left_corner", imageStore.getImageList("dirt_bot_left_corner")));
        }
        else if (world.getBackgroundCell(current).getImages().equals(imageStore.getImageList("magic_dirt_bot_right_up"))) {
            world.setBackgroundCell(current, new Background("dirt_bot_right_up", imageStore.getImageList("dirt_bot_right_up")));
        }
        else if (world.getBackgroundCell(current).getImages().equals(imageStore.getImageList("magic_dirt_horiz"))) {
            world.setBackgroundCell(current, new Background("dirt_horiz", imageStore.getImageList("dirt_horiz")));
        }
        else if (world.getBackgroundCell(current).getImages().equals(imageStore.getImageList("magic_dirt_vert_right"))) {
            world.setBackgroundCell(current, new Background("dirt_vert_left", imageStore.getImageList("dirt_vert_left")));
        }
        else if (world.getBackgroundCell(current).getImages().equals(imageStore.getImageList("magic_dirt_vert_left_bot"))) {
            world.setBackgroundCell(current, new Background("dirt_vert_left_bot", imageStore.getImageList("dirt_vert_left_bot")));
        }
        else if (world.getBackgroundCell(current).getImages().equals(imageStore.getImageList("magic_dirt_vert_left"))) {
